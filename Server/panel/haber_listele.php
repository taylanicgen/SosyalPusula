...
if($_GET['islem']=="sil")
{
	...
	$resim_bul=mysql_fetch_array(mysql_query("select * from haberler where id='$id'"));
	$resim_sil=unlink("../lib/images/haberler/".$resim_bul['resim']);
	
	$kategori_sil_sorgu = mysql_query("delete from haberler where id='$id'");
               ...
               <div class="span12">
                  ...
                  <div class="widget box light-grey">
                     <div class="widget-title">
                        <h4><i class="icon-cloud"></i> Haber Listesi</h4>
                     ...
                     <div class="widget-body">
                        <div class="clearfix margin-bottom-10">
                           <div class="btn-group">
                              ...
                              <a href="haber_ekle.php" style="color:#FFF;">Haber Ekle</a> <i class="icon-plus"></i>
                              </div>
                           </div>
                        ...
                        <table class="table table-striped table-bordered table-hover" id="sample_1" style="font-size:12px;">
                           <thead>
                              <tr>
                                 ...
                                 <th> Haber Adı <i class="icon-angle-down" style="float:right;"></th>
                                 <th class="hidden-480">Haber Resmi <i class="icon-angle-down" style="float:right;"></th>
                           ...
                           <?php
						    $haber_cek=mysql_query("select * from haberler order by sira asc");
							while($haber_dizi=mysql_fetch_array($haber_cek))
							{
						   ?>
                           
                              <tr class="odd gradeX">
                                 <td><?=$haber_dizi['sira']?></td>
                                 <td><?=$haber_dizi['adi']?></td>
                                 <td>
                                 <?php
								 if($haber_dizi['resim']=="no-image")
								 {
								 echo "Resim yok";
								 }
								 else
								 {
								 ?>
                                 <a href="../lib/images/haberler/<?=$haber_dizi['resim']?>" target="_blank">Resmi Gör</a>
                                 <?php
								 }
								 ?>
                                 ...
                                 <td >
                                 <a href="?islem=sil&id=<?=$haber_dizi['id']?>" title="Sil" onclick="return confirm('Silmek istediğinize emin misiniz ?')">
                                 <i class="icon-remove sercan_sil_icon"></i>
                                 </a>
                                 
                                 <a href="haber_ekle.php?islem=duzenle&id=<?=$haber_dizi['id']?>" title="Düzenle">
